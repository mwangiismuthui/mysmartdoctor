...
<div class="card">
    <h6 class="card-header">{{ __(' Chat List') }}</h6>
            ...
            <table class="table table-striped table-hover" id="table" style="width:100%;">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            {{ Auth::user()->role == 'patient' ? \App\User::find($item->from)->patient->first_name :'' }}
                            {{ Auth::user()->role == 'doctor' ? \App\User::find($item->from)->doctor->given_name :''}}
                        </td>
                        <td>
                            {{ Auth::user()->role == 'patient' ? \App\User::find($item->to)->doctor->given_name :'' }}
                            {{ Auth::user()->role == 'doctor' ? \App\User::find($item->to)->patient->first_name :''}}
                        </td>
                        <td>{!! $item->content !!}</td>
